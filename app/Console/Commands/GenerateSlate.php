...
class GenerateSlate extends Command
{
    ...
    public function generateSlate(Channel $channel): bool
    {
        ...
        if ($code !== 0) {
            Log::error("Slate generation failed for [{$channel->name}]: " . implode("\n", $out));
    ...
    {
        // ffmpeg drawtext: avoid single quotes (which conflict with drawtext's '...' quoting)
        // by replacing them with the Unicode right single quotation mark
        $text = str_replace("'", "\u{2019}", $text);

        // escape colons and backslashes that could break option parsing
